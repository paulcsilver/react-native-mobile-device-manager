...
- (void)settingsDidChange:(NSDictionary<NSString *, id> *)changes
{
    ...
    [self sendEventWithName:APP_CONFIG_CHANGED body:appConfig];
...
- (void)guidedAccessStatusChangeListenerCallback:(NSNotification*)notification
{
    ...
    dispatch_async(_eventQueue, ^{
        if (self->_guidedAccessCallbackRequired != NO) {
            ...
            [self isSAMEnabled:^(BOOL isEnabled) {
                [self isASAMSupported:^(BOOL isAllowed) {
                    ...
                    [self sendEventWithName:APP_LOCK_STATUS_CHANGED body:(@{
                    APP_LOCKED: @(isEnabled),
                    APP_LOCKING_ALLOWED: @(isAllowed)
                    })];
//                    [self->_bridge.eventDispatcher sendDeviceEventWithName:APP_LOCK_STATUS_CHANGED
//                                                                body:(@{
//                                                                        APP_LOCKED: @(isEnabled),
//                                                                        APP_LOCKING_ALLOWED: @(isAllowed)
//                                                                        })];
...
- (void) isASAMSupported:(void(^)(BOOL))callback
{
    ...
    void (^onComplete)(BOOL success) = ^(BOOL success){
        self->_guidedAccessCallbackRequired = YES;
        callback(success);
    };
...
{
  return @[APP_CONFIG_CHANGED, APP_LOCK_STATUS_CHANGED];
}
...
RCT_EXPORT_METHOD(sampleMethod:(NSString *)stringArgument numberParameter:(nonnull NSNumber *)numberArgument callback:(RCTResponseSenderBlock)callback)
{
    ...
    callback(@[[NSString stringWithFormat: @"numberArgument:\n%@\n\nstringArgument:\n%@", numberArgument, stringArgument]]);
}
